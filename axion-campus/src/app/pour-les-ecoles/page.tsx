...
/**
 * Landing Écoles — src/app/pour-les-ecoles/page.tsx  (NOUVEAU)
 *
 * Reprend le contenu de l'ancienne src/app/page.tsx (la landing école dense)
 * mais restructuré pour :
 * - Coexister avec la nouvelle page d'accueil hub (/page.tsx)
 * - Ajouter un lien "Retour à l'accueil"
 * - Intégrer SiteFooter complet
 * - Préserver toutes les sections existantes : hero, CurriculumSection,
 *   LearningOutcomes, SocialProof
 */
    ...
    <svg className="absolute right-0 top-0 opacity-[0.06] pointer-events-none" width="380" height="220" viewBox="0 0 380 220">
      {coords.map(([cx,cy],i) => (
        <polygon key={i}
          ...
          strokeWidth={i%3===0?'1.5':'0.8'} strokeOpacity={i%2===0?'1':'0.5'}/>
      ))}
    </svg>
  )
}
      ...
      <section className="relative min-h-[calc(100vh-56px)] flex flex-col items-center justify-center px-5 pt-24 pb-16 overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-4xl mx-auto text-center flex flex-col items-center">
          <div className="inline-flex items-center gap-2 bg-[#0A66C2]/10 border border-[#0A66C2]/30 rounded-full px-4 py-1.5 mb-8">
            ...
            <span className="text-xs text-[#7DD3FC] font-semibold tracking-widest uppercase">Pour les écoles · AXION Campus</span>
          </div>
          ...
          <h1 className="text-5xl sm:text-6xl md:text-7xl font-black text-white leading-[1.02] mb-6 tracking-tight"
            ...
            <span className="relative inline-block">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#0A66C2] via-[#38BDF8] to-[#7DD3FC]">promotion IA</span>
          ...
          <p className="text-slate-300 text-xl sm:text-2xl max-w-2xl leading-relaxed mb-10">
            Donnez à vos étudiants un score objectif sur leur maturité IA agentique.
            Dashboard admin, rapport de promotion, zéro installation.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 mb-16">
            <button onClick={() => router.push('/inscription')}
              className="group bg-[#0A66C2] hover:bg-[#0958a8] text-white font-bold px-8 py-4 rounded-2xl text-base transition-all hover:scale-[1.03] hover:shadow-2xl hover:shadow-[#0A66C2]/30 flex items-center gap-2">
      ...
      <section className="px-5 py-12 max-w-3xl mx-auto w-full">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button onClick={() => router.push('/connexion')}
            className="group relative bg-[#0A66C2] hover:bg-[#0958a8] rounded-2xl p-7 text-left transition-all hover:scale-[1.02] hover:shadow-2xl hover:shadow-[#0A66C2]/30">
            <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center mb-4">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8"><path d="M12 14l9-5-9-5-9 5 9 5z"/></svg>
            </div>
            ...
            <p className="text-white/60 text-sm">Entrez votre code d'accès fourni par votre école.</p>
          ...
          <button onClick={() => router.push('/admin')}
            className="group relative bg-[#0d1b2e] hover:bg-[#111f33] border border-slate-700 hover:border-[#0A66C2]/50 rounded-2xl p-7 text-left transition-all hover:scale-[1.02]">
            ...
            <p className="text-slate-400 text-sm">KPIs, résultats et satisfaction de votre promo.</p>
