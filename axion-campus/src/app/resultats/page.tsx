...
function ShareSection({ studentName, score, grade, certUid, sessionId }: {
  ...
  // URL publique du certificat (lien unique)
  ...
  const copyLink = async () => {
    try { await navigator.clipboard.writeText(certUrl); setCopied(true); setTimeout(() => setCopied(false), 2500); } catch { /* fallback */ }
  };
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8">
      ...
      <p className="text-sm text-slate-500 mb-6 leading-relaxed">
        Valorisez votre niveau IA sur vos réseaux professionnels et ajoutez cette certification à votre profil LinkedIn.
      </p>

      {/* LinkedIn Add to Profile — bouton principal */}
      <a
        href={linkedinAddUrl}
        target="_blank"
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <a href={linkedinShareUrl} target="_blank" rel="noopener noreferrer"
          ...
          <span className="text-xs text-slate-600 group-hover:text-[#0A66C2] font-medium">LinkedIn</span>
        </a>
        ...
        <a href={twitterUrl} target="_blank" rel="noopener noreferrer"
          ...
          <span className="text-xs text-slate-600 group-hover:text-slate-900 font-medium">X / Twitter</span>
        </a>

        {/* Copier le lien */}
        <button onClick={copyLink}
          className={`flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border transition-all ${copied ? 'border-green-300 bg-green-50' : 'border-slate-200 hover:border-slate-400 hover:bg-slate-50'}`}>
          {copied ? (
...
export default function ResultatsPage() {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      // Colonne gauche
      pdf.setTextColor(255,255,255); pdf.setGState(new (pdf as any).GState({ opacity: 0.45 }));
      ...
      if (segs > 0) {
        ...
        for (let i=0;i<=segs;i++){const a=(Math.PI*2*i)/60-Math.PI/2;arcPts.push([cx+outerR*Math.cos(a),cy+outerR*Math.sin(a)]);}
        for (let i=segs;i>=0;i--){const a=(Math.PI*2*i)/60-Math.PI/2;arcPts.push([cx+innerR*Math.cos(a),cy+innerR*Math.sin(a)]);}
        pdf.setFillColor(125,211,252);
        pdf.lines(arcPts.slice(1).map((p,i)=>[p[0]-arcPts[i][0],p[1]-arcPts[i][1]] as [number,number]),arcPts[0][0],arcPts[0][1],[1,1],"F",true);
      }
      pdf.setFontSize(9); pdf.setFont("helvetica","bold"); pdf.setTextColor(255,255,255);
      pdf.text(`${score}`, cx, cy-1, {align:"center"});
      pdf.setFontSize(5.5); pdf.setFont("helvetica","normal"); pdf.setTextColor(180,210,255);
      ...
      pdf.setFontSize(7); pdf.setTextColor(160,195,240); pdf.setFont("helvetica","normal");
      ...
      pdf.setFontSize(34); pdf.setFont("helvetica","bold"); pdf.setTextColor(255,255,255);
      ...
      data.dimensions.forEach((dim, i) => {
        const y = 88+i*16;
        pdf.setFontSize(7); pdf.setFont("helvetica","normal"); pdf.setTextColor(210,230,255);
        pdf.text(dim.label, rx, y);
        pdf.setFont("helvetica","bold"); pdf.setTextColor(125,211,252);
        pdf.text(`${dim.score}%`, rx+barW, y, {align:"right"});
        pdf.setFillColor(255,255,255); pdf.setGState(new (pdf as any).GState({ opacity: 0.1 }));
        pdf.roundedRect(rx, y+2, barW, 5, 2, 2, "F"); pdf.setGState(new (pdf as any).GState({ opacity: 1 }));
        const filled = (dim.score/100)*barW;
        if (filled>0) { pdf.setFillColor(125,211,252); pdf.roundedRect(rx, y+2, filled, 5, 2, 2, "F"); }
      });
      pdf.setFontSize(6); pdf.setFont("helvetica","normal"); pdf.setTextColor(160,195,240);
    ...
    } catch(err) { console.error(err); alert("Erreur génération PDF."); } finally { setPdfLoading(false); }
  };
        ...
        <div className="bg-white rounded-[32px] shadow-2xl border border-slate-200 overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8">
              <div>
                ...
                <p className="text-blue-100">
                  {new Date(data.completed_at).toLocaleDateString("fr-FR",{day:"numeric",month:"long",year:"numeric"})}
                </p>
          ...
          <div className="px-10 py-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              {/* Carte certificat + téléchargement */}
              <div className="bg-slate-50 rounded-3xl border border-slate-200 p-8 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-slate-600 leading-relaxed text-sm">
                    Ce document atteste officiellement de vos résultats. Le certificat est généré avec un identifiant unique et vérifiable.
                  </p>
