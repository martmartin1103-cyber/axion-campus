...
function calcScore(reponses: Record<string, string>) {
  ...
  return { d1:scores[0], d2:scores[1], d3:scores[2], d4:scores[3], d5:scores[4], global, grade: grade(global) }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: session, error: sessErr } = await sb
      .from('sessions')
      .insert({ passe_id, ecole_id, nom_etudiant, promo, reponses, statut: 'termine', finished_at: new Date().toISOString() })
      .select().single()
    if (sessErr) throw sessErr

    // Calculer et enregistrer les résultats
    ...
    const { error: resErr } = await sb.from('resultats').insert({
      session_id: session.id, ecole_id,
      score_global: scores.global, grade: scores.grade,
      score_d1: scores.d1, score_d2: scores.d2, score_d3: scores.d3,
      score_d4: scores.d4, score_d5: scores.d5, cert_uid
    })
    if (resErr) throw resErr

    // Marquer la passe comme utilisée
    await sb.from('passes').update({ utilise: true }).eq('id', passe_id)

    return NextResponse.json({ session_id: session.id, cert_uid, ...scores })
  } catch (err) {
